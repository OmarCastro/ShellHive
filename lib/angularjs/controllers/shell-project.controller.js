...
function shellProjectController($scope, csrf, alerts, modal, $timeout) {
    ...
    viewGraph = $scope.viewGraph = function viewGraph(graph) {
        ...
        router_1.default.send({
            payload: { id: graph },
            useRoute: router_1.routeTable.graphSubscription()
        }).onResponse(function (res) { return $scope.$applyAsync(function () {
            console.log(res);
    ...
    router_1.default.send({
        payload: { id: utils_1.projectId },
        useRoute: router_1.routeTable.projectSubscription()
        ...
        if (data.visitor) {
            ...
            if (visitorName && visitorColor) {
                router_1.default.send({ payload: { name: visitorName, color: visitorColor }, useRoute: router_1.routeTable.setUserName() });
            }
            else {
                welcomeUserModal.showModal(modal).then(function (form) {
                    router_1.default.send({ payload: { name: form.name }, useRoute: router_1.routeTable.setUserName() });
        ...
        if (len == 0) {
            return projectCreationModal.showModal(modal).then(function (form) {
                ...
                io.socket.post('/graph/createfromcommand/', {
                    project: utils_1.projectId, type: 'root',
