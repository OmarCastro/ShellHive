...
function shellProjectController($scope, csrf, alerts, modal, $timeout) {
    ...
    viewGraph = $scope.viewGraph = function viewGraph(graph) {
        ...
        router_1.default.subscribeGraph({ id: graph }).request(function (res) { return $scope.$applyAsync(function () {
            console.log(res);
    ...
    router_1.default.subscribeProject({ id: utils_1.projectId }).request(function (data) {
        ...
        if (data.visitor) {
            ...
            if (visitorName && visitorColor) {
                router_1.default.setUserName({ name: visitorName, color: visitorColor }).request;
            }
            else {
                var form_1 = { name: '' };
                ...
                modalInstance.result.then(function (selectedItem) {
                    router_1.default.setUserName({ name: form_1.name, _csrf: csrf.csrf }).request;
