...
function shellProjectController($scope, csrf, alerts, modal, $timeout) {
    ...
    viewGraph = $scope.viewGraph = function viewGraph(graph) {
        ...
        router_1.default.get('/graph/subscribe/', { id: graph }, function (res) {
            console.log(res);
            $scope.graphData.components = res.graph.components.map(function (component) {
                ...
                res.graph.connections.forEach(function (connection) {
                    if (connection.startNode == component.id)
                        connection.startComponent = component.data;
                    if (connection.endNode == component.id)
                        connection.endComponent = component.data;
                });
    ...
    router_1.default.get('/project/subscribe', { id: utils_1.projectId }, function (data) {
        $scope.implementedCommands = data.implementedCommands;
        $scope.options = data.SelectionOptions;
        $scope.clients = data.clients;
        $scope.page = "graph";
        $scope.chatterId = socket_service_1.SocketService.socket.id;
        $scope.chat = { open: $scope.clients.length > 1 };
        console.log("socket.id =", $scope.chatterId);
        $scope.$digest();
        if (data.visitor) {
            var visitorName = utils_1.projectId == "3" ? "anon" : sessionStorage["visitorName"];
            var visitorColor = utils_1.projectId == "3" ? "blue" : sessionStorage["visitorColor"];
            if (visitorName && visitorColor) {
                router_1.default.post('/project/setmyname', { name: visitorName, color: visitorColor });
            }
            else {
                var form_1 = { name: '' };
                var modalInstance = modal.open({
                    ...
                    controller: function ($scope, $modalInstance) {
                        $scope.form = form_1;
                        $scope.ok = function () {
                            $modalInstance.close(form_1);
                        };
                    }
                });
                modalInstance.result.then(function (selectedItem) {
                    router_1.default.post('/project/setmyname', { name: form_1.name, _csrf: csrf.csrf });
                    sessionStorage["visitorName"] = form_1.name;
        ...
        if (len == 0) {
            ...
            var modalInstance = modal.open({
                ...
                controller: function ($scope, $modalInstance) {
                    $scope.form = form_2;
                    $scope.ok = function () {
                        $modalInstance.close(form_2);
                    };
                }
            });
            return modalInstance.result.then(function (selectedItem) {
                console.log(selectedItem);
                io.socket.post('/graph/createfromcommand/', { project: utils_1.projectId, type: 'root',
                    command: form_2.command, _csrf: csrf.csrf }, function (res) {
                    console.log(res);
                    if (res.graph) {
                        var graph = res.graph;
    ...
    io.socket.on('stdout', function (data) {
        ...
        if ($scope.shellText.length > 100) {
            $scope.shellText = $scope.shellText.slice(-100);
        }
        $scope.$digest();
    });
    ...
    io.socket.on('retcode', function (data) {
        $scope.shellText.push({
            text: "command finished with code " + data,
            type: "call"
        });
        $scope.$digest();
    });
