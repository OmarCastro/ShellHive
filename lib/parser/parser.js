...
function aux_parseVisualDataExperimental(VisualData, fifoPrepend) {
    ...
    VisualData.components.forEach(function (component) {
        ...
        if (portList.list.length == 1 && keys[0] == "input" && component.type != exports.MacroComponent.type && (!component["files"] || component["files"].length == 0)) {
            ...
            if (prevPortList.list.length == 1 && prevPortList.byPortList["output"]) {
                if (component.type == exports.CommandComponent.type) {
                    ...
                    return;
                } else if (component.type == exports.FileComponent.type) {
                    redirect[component.id] = {
                        "as": "overwrite",
    ...
    VisualData.components.forEach(function (component) {
        ...
        function appendSingleOutput(parsedExec, output) {
            if (redirect[output.endNode]["as"] == "pipe") {
                ...
                redirect[output.endNode].commandIndex = (componentRedirect["as"] == "pipe" && componentRedirect.commandIndex != null) ? componentRedirect.commandIndex : commands.length;
            } else if (redirect[output.endNode]["as"] == "overwrite") {
                parsedExec += " > " + redirect[output.endNode].file;
            } else {
                parsedExec += " > " + fifoPrepend + output.endNode + "-" + output.endPort;
            }
            return parsedExec;
        }
...
function parseVisualDataExperimental(VisualData, fifoPrepend) {
    ...
    if (result.fifos.length < 1) {
        ...
        if (result.commands.length <= 1) {
            ...
            pretty_printed_commands = result.commands.map(function (c) {
                return c + " &";
            }).join("\n");
        }
    } else {
        ...
        pretty_printed_commands = [mkfifos].concat(result.commands.map(function (c) {
            return c + " &";
        })).join("\n");
    }
    return {
