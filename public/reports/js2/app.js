...
*/
var cssTransform, SelectionOptions, examplesControllerData, visual2textControllerData, AST_tests, app;
...
function isImplemented(data){
  return ['awk', 'cat', 'grep', 'bunzip2', 'bzcat', 'bzip2', 'compress', 'ls', 'gzip', 'gunzip', 'zcat'].indexOf(data.exec) >= 0;
}
...
app.directive("connector", function($document){
  return {
    ...
    link: function(scope, element, attr){
      ...
      elem = element[0];
      graphElement = elem.parentElement.parentElement.parentElement;
...
app.directive("graph", function($document){
  ...
  return {
    scope: false,
    link: function(scope, element, attr){
      var scale, graphX, graphY, startX, startY, elem, nodesElem, edgesElem, svgElem, jqel, nodesElemStyle, edgesElemStyle, mousemove, mouseup, multScale, MouseWheelHandler, mousewheelevt;
      scope.transform = cssTransform.replace(/[A-Z]/g, function(v){
        return "-" + v.toLowerCase();
      });
      scale = 1;
      ...
      scope.translateNode = function(id, position, x, y){
        ...
        position.y += y / scale;
        scope.$digest();
        for (i$ = 0, len$ = (ref$ = edgesElem.querySelectorAll("path")).length; i$ < len$; ++i$) {
          el = ref$[i$];
          results$.push(angular.element(el).scope().updateWithId(id));
        }
        return results$;
      };
    }
